  ...
  <div class="flex-shrink max-w-full px-4 w-full mb-6">
    <div class="p-4 bg-white dark:bg-gray-800 rounded-lg shadow-lg h-full text-gray-700">
      <div class="relative p-4 overflow-x-auto">
        ...
        <x-sweet-action-notif model="Role"></x-sweet-action-notif>
        ...
        <div class="mb-12">
          ...
          <div class="flex justify-start">
            <a href="{{ route('sysadmin.credential.roles.create') }}" class="py-2 px-4 inline-block text-center mb-3 rounded leading-5 text-gray-100 bg-green-500 border border-green-500 hover:text-white hover:bg-green-600 hover:ring-0 hover:border-green-600 focus:bg-green-600 focus:border-green-600 focus:outline-none focus:ring-0 mr-2">
          ...
          <table class="table-auto w-full">
            <thead class="bg-gray-100 border-t border-l border-r dark:bg-gray-900 dark:bg-opacity-40 dark:border-gray-700">
              <tr>
                ...
                <th class="p-2 min-w-[150px]">Guard Name</th>
              ...
              @foreach ($roles as $role)
                <tr>
                  <td class="border border-gray-200 p-2 dark:border-gray-700 text-center">
                    <a href="{{ route('sysadmin.credential.roles.edit', $role->id) }}" class="inline-block text-blue-500 hover:text-blue-700 border border-blue-500 hover:border-blue-700 rounded">
                      {{-- Heroicons -- Pencil --}}
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                        <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                      </svg>
                    </a>
                    <form action="{{ route('sysadmin.credential.roles.destroy', $role->id) }}" id="form{{ $role->id }}" method="POST" class="inline-block">
                      @csrf
                      @method('DELETE')
                      <button type="submit" x-on:click="confirm('form{{ $role->id }}')" class="inline-block text-red-500 hover:text-red-700 border border-red-500 hover:border-red-700 rounded">
                        {{-- Heroicons -- Trash --}}
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
