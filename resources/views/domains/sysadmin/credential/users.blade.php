  ...
  <div class="flex-shrink max-w-full px-4 w-full mb-6">
    <div class="p-4 bg-white dark:bg-gray-800 rounded-lg shadow-lg h-full text-gray-700">
      <div class="relative p-4 overflow-x-auto">
        ...
        <x-sweet-action-notif model="User"></x-sweet-action-notif>
              ...
              @foreach ($users as $user)
                <tr>
                  <td class="border border-gray-200 p-2 dark:border-gray-700 text-center">
                    ...
                    <form action="{{ route('sysadmin.credential.users.destroy', $user->id) }}" id="form{{ $user->id }}" method="POST" class="inline-block">
                      @csrf
                      @method('DELETE')
                      <button type="submit" x-on:click="confirm('form{{  $user->id }}')" class="inline-block text-red-500 hover:text-red-700 border border-red-500 hover:border-red-700 rounded">
                        {{-- Heroicons -- Trash --}}
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
