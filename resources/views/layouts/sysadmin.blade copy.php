...
@section('level1')
  ...
  <div x-data="{ open: false }" class="overflow-x-hidden bg-slate-900 bg-opacity-40">
    <nav id="sidebar-menu" x-description="Mobile menu" x-bind:aria-expanded="open" :class="{ 'w-64 md:w-0': open, 'w-0 md:w-64': !(open) }" class="fixed transition-all duration-500 ease-in-out h-screen bg-slate-900 shadow-sm w-0 md:w-64" aria-expanded="false">
      <div class="h-full overflow-y-auto scrollbars">
        ...
        <!-- Sidebar menu -->
        <ul x-data="{ level1: '{{ (Str::title(request()->segment(2))) }}' }" class="w-full float-none flex flex-col font-normal pl-1.5 text-zinc-400">
          @foreach ($menus as $item1)
            @if ($item1->type === 'dropdown')
              <li class="relative">
                ...
                <ul x-data="{ level2: '{{ (Str::title(request()->segment(3))) }}' }" x-show="level1 == '{{ $item1->name }}'" class="pl-4">
                  @foreach ($item1->childs as $item2)
                    @if ($item2->type === 'dropdown')
                      <li class="relative">
                        ...
                        <ul x-data="{ level2: null }" x-show="level2 == '{{ $item2->name }}'" class="pl-4">
    ...
    {{-- Header Menu --}}
    <div x-bind:aria-expanded="open" :class="{ 'ml-64 -mr-64 md:ml-0 md:mr-0': open, 'ml-0 mr-0 md:ml-64': !(open) }" class="flex flex-col min-h-screen transition-all duration-500 ease-in-out ml-0 mr-0 md:ml-64" aria-expanded="false">
      {{-- Navbar --}}
      <nav :class="{ 'left-64 -right-64 md:left-0 md:right-0': open, 'left-0 right-0 md:left-64': !(open) }" class="z-50 fixed flex flex-row flex-nowrap items-center justify-between mt-0 py-2 px-6 bg-white shadow-sm transition-all duration-500 ease-in-out left-0 right-0 md:left-64">
        ...
        <form class="hidden sm:inline-block md:hidden lg:inline-block mx-5">
          <div class="flex flex-wrap items-stretch w-full relative">
            <input type="text" class="flex-shrink flex-grow max-w-full leading-5 relative text-sm py-2 px-4 rounded-l text-gray-800 bg-gray-100 overflow-x-auto focus:outline-none border border-gray-100 focus:border-gray-200 focus:ring-0 dark:text-gray-400 dark:bg-gray-700 dark:border-gray-700 dark:focus:border-gray-600" placeholder="Search…" aria-label="Search">
            <div class="flex -mr-px">
              <button class="flex items-center py-2 px-4 -ml-1 leading-5 text-gray-100 bg-indigo-500 border border-indigo-500 hover:text-white hover:bg-indigo-600 hover:ring-0 hover:border-indigo-600 focus:bg-indigo-600 focus:border-indigo-600 focus:outline-none focus:ring-0" type="button">
        ...
        {{-- Menu --}}
        <ul class="flex ml-auto mt-0">

          <!-- messages -->
          <li x-data="{ open: false }" class="relative">
            <a href="#" class="py-3 px-4 flex text-sm rounded-full focus:outline-none" id="messages" x-on:click="open = ! open">
              ...
              <div class="p-3 font-normal border-b border-gray-200 dark:border-gray-700">
                <div class="relative">
                  ...
                  <div x-data="{ open: false }" class="absolute top-0 right-0">
                    <a @click="open = ! open" href="javascript:;" class="inline-block mr-2 " title="Search message">
                    ...
                    <div x-show="open" @click.away="open = false" class="origin-top-right absolute right-0 bg-white dark:bg-gray-700 z-10 rounded" style="min-width: 16rem; display: none;">
                    ...
                    <a href="#" class="inline-block mr-2" title="Mark all as read">
                    ...
                    <a href="#" class="inline-block mr-2" title="New message">
              ...
              <div class="max-h-60 overflow-y-auto scrollbars show">
                  ...
                  <div class="flex flex-wrap flex-row items-center border-b border-gray-200 dark:border-gray-700 dark:bg-gray-900 dark:bg-opacity-40 dark:hover:bg-opacity-20 py-2 hover:bg-gray-100 bg-gray-50">
                    <div class="flex-shrink max-w-full px-2 w-1/4 text-center">
                      <div class="relative">
                        ...
                        <span title="offline" class="flex justify-center absolute -bottom-0.5 right-2 text-center bg-gray-500 border border-white w-3 h-3 rounded-full"></span>
                      </div>
                    </div>
                    <div class="flex-shrink max-w-full px-2 w-3/4">
                ...
                <a href="#">
                  <div class="flex flex-wrap flex-row items-center border-b border-gray-200 dark:border-gray-700 dark:hover:bg-gray-900 dark:hover:bg-opacity-20 py-2 hover:bg-gray-100">
                    <div class="flex-shrink max-w-full px-2 w-1/4 text-center">
                      <div class="relative">
                        <img src="{{ asset('assets/img/profile.png') }}" class="h-10 w-10 rounded-full mx-auto" alt="Roman Davis">
                        <span title="offline" class="flex justify-center absolute -bottom-0.5 right-2 text-center bg-gray-500 border border-white w-3 h-3 rounded-full"></span>
                      </div>
                    </div>
                    <div class="flex-shrink max-w-full px-2 w-3/4">
          ...
          <li x-data="{ open: false }" class="relative">
            ...
            <div x-show="open" @click.away="open = false" x-transition:enter="transition-all duration-200 ease-out" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition-all duration-200 ease-in" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="w-72 origin-top-right absolute right-0 rounded top-full z-50 py-0.5 text-left bg-white dark:bg-gray-800 border dark:border-gray-700 shadow-md" style="display: none;">
              <div class="p-3 font-normal border-b border-gray-200 dark:border-gray-700">
                <div class="relative">
                  <div class="font-bold">Notifications</div>
                  <div class="absolute top-0 right-0">
                    <a href="#" class="inline-block mr-2" title="Clear all">
          ...
          <li x-data="{ open: false }" class="relative">
            <a href="javascript:;" class="px-3 flex text-sm rounded-full focus:outline-none" id="user-menu-button" @click="open = ! open">
              <div class="relative">
                <img class="h-10 w-10 rounded-full border border-gray-700 bg-gray-700" src="{{ asset('assets/img/profile.png') }}" alt="avatar">
              ...
              <span class="hidden md:block ml-1 self-center">Ari Budin</span>
            </a>
            <ul x-show="open" @click.away="open = false" x-transition:enter="transition-all duration-200 ease-out" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition-all duration-200 ease-in" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="origin-top-right absolute right-0 rounded top-full z-50 py-0.5 text-left bg-white dark:bg-gray-800 border dark:border-gray-700 shadow-md" style="min-width:12rem;display: none;">
              <li class="relative">
                <div class="flex flex-wrap flex-row -mx-4 px-3 py-4 items-center">
                  <div class="flex-shrink max-w-full px-4 w-1/3">
                    <img src="{{ asset('assets/img/profile.png') }}" class="h-10 w-10 rounded-full" alt="avatar">
                  </div>
                  <div class="flex-shrink max-w-full px-4 w-2/3 pl-1">
              ...
              <li class="relative">
                <a class="block w-full py-2 px-6 clear-both whitespace-nowrap hover:text-indigo-500" href="#">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="inline mr-2 w-4 h-4 bi bi-gear" viewBox="0 0 16 16">
              ...
              <li class="relative">
                <a class="block w-full py-2 px-6 clear-both whitespace-nowrap hover:text-indigo-500" href="#">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="inline mr-2 w-4 h-4 bi bi-question-circle" viewBox="0 0 16 16">
              ...
              <li class="relative">
                <a class="block w-full py-2 px-6 clear-both whitespace-nowrap hover:text-indigo-500" href="#">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="inline mr-2 w-4 h-4 bi bi-translate" viewBox="0 0 16 16">
              ...
              <li class="relative">
                <a class="block w-full py-2 px-6 clear-both whitespace-nowrap hover:text-indigo-500" href="#">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="inline mr-2 w-4 h-4 bi bi-box-arrow-in-right" viewBox="0 0 16 16">
